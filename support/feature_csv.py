...
from modelTraining.preprocessing import featureExtract
from modelTraining.preprocessing.segmentazione import rimuovi_ripetizioni


def crea_csv(path_video):
    #print("chiamata rimuovi rip con path "+ path_video)
    #norm_path_video = os.path.normpath(path_video)
    #print(f"Norm path video: {norm_path_video}")
    #new_path_video = rimuovi_ripetizioni(norm_path_video)
    csv_file = open("support/csv/dataset.csv", "w", newline="")
        ...
        with mp_pose.Pose(static_image_mode=False, model_complexity=2, min_detection_confidence=0.5) as pose:
            ...
            while cap.isOpened():
                ...
                if frame_num % interval == 0:
                    ...
                    XrightShoulder = results.pose_landmarks.landmark[
                                         mp_pose.PoseLandmark.RIGHT_SHOULDER].x * image_width
                    YrightShoulder = results.pose_landmarks.landmark[
