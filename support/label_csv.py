...
def crea_csv(path_xlsx):

    # Apri il file CSV in modalità scrittura
    with open('support/csv/labelAusiliare.csv', mode='w', newline='') as file:
        mediaHappy = 0
        countHappy = 0
        mediaAngry = 0
        countAngry = 0
    ...
    with open('support/csv/labelAusiliare.csv', 'r') as input_file, open('support/csv/label.csv', 'w', newline='') as output_file:
    ...
    with open("support/csv/label.csv") as csv_file:
    ...
    plt.bar(emotion_counts.keys(), emotion_counts.values(),
            color=[colors[emotion] for emotion in emotion_counts.keys()])
    plt.xlabel("Emozione")
    plt.ylabel("Numero")
